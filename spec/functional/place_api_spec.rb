...
def get_base_uri
  "http://#{ENV['WEB_PORT_3000_TCP_ADDR']}:#{ENV['WEB_PORT_3000_TCP_PORT']}"
end
...
describe 'When adding a place to the list' do
  it 'should return 201 created' do
    ...
    expect(response.headers['location']).to match(/#{get_base_uri}\/place\/\d+/)
...
def add_place(google_id)
  HTTParty.post("#{get_base_uri}/place",
...
def delete_place(place_id)
  HTTParty.post("#{get_base_uri}/place/#{place_id}/delete", { :follow_redirects => false })
end

describe 'When removing a place from the list' do
  it 'should be achievable via POST method which redirects to index' do
    response = HTTParty.post("#{get_base_uri}/place/12345/delete", { :follow_redirects => false })

    expect(response.code).to eq(302)
    expect(response.headers['location']).to eq("#{get_base_uri}/")
  end
end
